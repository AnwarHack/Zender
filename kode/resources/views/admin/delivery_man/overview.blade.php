                        ...
                        <div class="card-header border-bottom-dashed ">
                         ...
                         @if($deliveryman->kyc_data)
                            <div class="card-body">
                                ...
                                <div class="p-3 bg-body rounded">
                                    <h6 class="mb-3 fw-bold">{{ translate('deliveryman KYC information') }}</h6>

                                    <ul class="list-group">

                                        @forelse($deliveryman->kyc_data as  $key => $data)
                                            <li
                                                class="d-flex justify-content-between align-items-center flex-wrap gap-2 list-group-item">
                                                <span class="fw-semibold">{{ k2t($key) }}</span>
                                                <span class="font-weight-bold">{{ $data->value }}</span>
                                                @if ($data->file)
                                                    <span class="fw-semibold">{{ translate('Attachment:') }}</span>

                                                    <a target="_blank"
                                                        href="{{ show_image(file_path()['delivery_man_kyc']['path'] . '/' . $data->file) }}">
                                                        <img alt='{{ $data->file }}'
                                                            class="mt-2 rounded  d-block avatar-xl img-thumbnail"
