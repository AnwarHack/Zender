...
class InhouseProductOrderController extends Controller
{
    ...
    public function orderStatusUpdate(Request $request, $id='')
    {
            ...
            if($order->status == 5){
                ...
                if($order->deliveryman){
                    ...
                    $transaction = Transaction::create([
                        ...
                        'amount'         => $order->amount,
                        'post_balance'     => $order->deliveryman->order_balance,
                    ...
                    $order->deliveryman->order_balance +=$order->amount;

                    $order->deliveryman->save();


                    if($order->delivery_man_charge > 0){

                        $transaction = Transaction::create([
                            'deliveryman_id'     => $order->deliveryman->id,
                            'amount'             =>$order->delivery_man_charge,
                            'post_balance'       => $order->deliveryman->balance,
                            'transaction_type'   => Transaction::PLUS,
                            'transaction_number' => trx_number(),
                            'details' => 'Order charge added for order  '.$order->order_id,
                        ]);

                        $order->deliveryman->balance +=$order->delivery_man_charge;
                        $order->deliveryman->save();
        ...
        if($order->deliveryman && @$order->deliveryman->fcm_token){
            $payload = (object) [
            ...
            $this->fireBaseNotification($order->deliveryman->fcm_token, $payload );
        }
    ...
    public function assign(Request $request){
        ...
        $order = Order::with(['deliveryman','customer'])->where('id', $request->id)->firstOrFail();


        $order->delivery_man_id = $request->delivery_man_id;
        $order->delivery_man_charge = $request->input("delivery_man_charge",0);
        $order->save();
        ...
        if($order->customer &&  $order->customer->fcm_token){
            $payload = (object) [
                "title"        => translate('Order'),
                "message"      => translate('Your order has been picked up by ').$order->deliveryman ? @$order->deliveryman->first_name : "Deliveryman",
                "order_number" => $order->order_id,
                "order_id"     => $order->id,
                "order_uid"    => $order->uid,
                "type"         => NotificationType::ORDER->value,
